<template>  
  ...
  >
    <v-card prepend-icon="mdi-home">
      <Show :data="showData" />
      <v-card-actions>
        <v-btn color="primary" variant="tonal" @click="infoDialog = false">Close</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
...
///// Edition in live mode
  ...
  const admin_api_exchangeuser = ["email", "is_staff", "is_superuser", "kyc", "kyc_reject_type", "withdrawals_count", "orders_count"]
  ...
  const ifFieldCanEdit = (key) => {
    ...
    if(param.value === "admin_api_exchangeuser") return !admin_api_exchangeuser.includes(key)
